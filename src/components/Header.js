...
export default function Header() {
  ...
  const getSearchInputClasses = (isMobile, isSearchExpanded) => {
    const baseClasses = "border-2 border-red-600 text-red-600 rounded-lg font-medium focus:outline-none focus:ring-0 bg-white placeholder-red-400";
  ...
  const SearchInput = ({ isMobile = false }) => {
    const containerClasses = `flex items-center ${
      (isMobile && isSearchExpanded) ? 'flex-1' : 
      (!isMobile && isSearchExpanded) ? 'w-full' : ''
    } ${isSearchExpanded ? 'gap-3' : ''}`;

    const searchIconClasses = `absolute ${
      isMobile 
        ? 'right-3 w-4 h-4 pointer-events-none' 
        : 'right-3 w-5 h-5'
  ...
  return (
    <header className="w-full">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        {/* Desktop Navigation */}
        <nav className={`hidden md:flex items-center ${searchTransition} ${
        ...
        <div className={`md:hidden flex items-center ${searchTransition} ${
