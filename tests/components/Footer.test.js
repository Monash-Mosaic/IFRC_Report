...
jest.mock('next-intl', () => ({
  useTranslations: (namespace) => (key) => {
    const translations = {
      ...
      'Home.footer.contributors.title': 'Contributors',
      'Home.footer.contributors.page1': 'Page',
      'Home.footer.contributors.page2': 'Page',
...
describe('Footer', () => {
  ...
  describe('Rendering', () => {
    it('renders the footer component correctly', () => {
      ...
      expect(screen.getAllByText('Contributors')).toHaveLength(2); // Mobile + Desktop
  ...
  describe('Mobile Dropdown Functionality', () => {
    it('shows mobile dropdown buttons on mobile', () => {
      ...
      expect(chevronIcons).toHaveLength(3); // One for each section
    });
    ...
    it('toggles contributors section dropdown', async () => {
      render(<Footer />);
      
      // Find the contributors dropdown button
      const contributorsButtons = screen.getAllByText('Contributors');
      const contributorsButton = contributorsButtons.find(button => button.closest('button'));
      expect(contributorsButton).toBeInTheDocument();
      
      // Click to expand
      await act(async () => {
        fireEvent.click(contributorsButton.closest('button'));
      });
      
      // Content should now be visible
      await waitFor(() => {
        const pageLinks = screen.getAllByText('Page');
        expect(pageLinks[0].closest('ul')).toHaveClass('block', 'mt-4');
      });
    });
  ...
  describe('Navigation Links', () => {
    it('renders all navigation links with correct href attributes', () => {
      ...
      // Contributor links
      const pageLinks = screen.getAllByText('Page');
      expect(pageLinks[0]).toHaveAttribute('href', '#contributors-page-1');
      expect(pageLinks[1]).toHaveAttribute('href', '#contributors-page-2');
    });
  ...
  describe('Responsive Behavior', () => {
    it('shows desktop headers with proper visibility classes', () => {
      ...
      const desktopHeader = allHeaders.find(header => 
        header.className.includes('hidden') && header.className.includes('md:block')
      );
    ...
    it('applies correct responsive layout classes', () => {
      ...
      const contentSection = screen.getByRole('contentinfo')
      ...
      const logoSection = screen.getByRole('contentinfo')
  ...
  describe('Logo Rendering', () => {
    it('renders IFRC logo with correct attributes', () => {
      ...
      expect(ifrcLogo).toHaveAttribute('src', '/wdr25/ifrc_logo.png');
    ...
    it('applies correct responsive alignment classes', () => {
      ...
      const logoSectionContainer = screen.getByRole('contentinfo')
  ...
  describe('Accessibility', () => {
    ...
    it('has proper heading hierarchy', () => {
      ...
      expect(sectionTitles).toHaveLength(6); // 3 desktop + 3 mobile headers
    });
  });
  ...
  describe('CSS Classes and Styling', () => {
    ...
    it('applies correct border and spacing classes', () => {
      ...
      const mobileButtons = screen.getAllByTestId('chevron-down-icon')
